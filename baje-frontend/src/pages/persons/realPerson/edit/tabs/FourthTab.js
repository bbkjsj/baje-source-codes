...
const FourthTab = (props) => {
  const [checkCodeStatus, setCheckStatus] = useState(
    props.job ? "edit" : "check"
  );
    ...
    <>
      <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
        <CheckCode
          label="عنوان شغل"
          name="job_title_id"
          job_Code={props.job && props.job.code}
          job_Title={props.job && props.job.title}
                ...
                onBlur: (e) => {
                  if (e.target.value && e.target.value.length === 8) {
                    checkInsuranceNumber(e.target.value)
                      ...
                      .catch(() => {
                        if (
                          props.form.getFieldValue("insurance_number") !=
                          props.insurance_number
                        ) {
