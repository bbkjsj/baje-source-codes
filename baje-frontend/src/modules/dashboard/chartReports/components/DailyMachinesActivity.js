...
function DailyMachinesActivity({ chartData }) {
      ...
      <Row justify="center" className="print-area">
        ...
        <Col xs={24} sm={24} md={24} lg={18} xl={18}>
          ...
          <CarouselWrapper arrows {...settings}>
            {chartData?.map((el) => (
              <Row justify="center">
                <Col span={24} className="justify-center flex-wrap mb-1">
                  {el.company_name + " " + el.contract_name}
                </Col>
                <Col span={24} span={24} className="justify-center flex-wrap">
                  {!el.active_car_quantity &&
                  !el.disabled_car_no_part_quantity &&
                  !el.disabled_car_no_tier_quantity ? (
                    <Col span={24}>
                      <EmptyChart></EmptyChart>
                    </Col>
                  ) : (
                    <Chart
                      key={el.contract_id_fk}
                      chartType="PieChart"
                      data={[
                        ["item", "value"],
                        ["  فعال ", parseFloat(el.active_car_quantity)],
                        [
                          "غیرفعال_نبود قطعه",
                          parseFloat(el.disabled_car_no_part_quantity),
                        ],
                        [
                          "غیرفعال_نبود تایر",
                          parseFloat(el.disabled_car_no_tier_quantity),
                        ],
                      ]}
                      width="100%"
                      height="500px"
...
export default DailyMachinesActivity;
