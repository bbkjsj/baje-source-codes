...
export const _POST_SUBORDINATE = (data) =>
  axios.post(api.postSubordinate, data);

export const _PUT = (id, data) => axios.put(api.put(id), data);

export const _PUT_SUBORDINATE = (id, data) =>
  axios.put(api.putSubordinate(id), data);

export const _DELETE = (ids) =>
  axios.delete(api.delete, { data: { ids: ids } });

export const _DELETE_SUBORDINATE = (ids) =>
  axios.delete(api.deleteSubordinate, {
    data: { ids: ids },
  });

export const _GET = (id) => axios.get(api.get(id) + "?page=1&size=5");

export const _GET_COMPANY_PERSONNEL = (currentOffice) => {
  return axios.get(api.getCompanyPersonnel(currentOffice));
};

export const _GET_BY_ID = (id, type) => axios.get(api.getByTypeId(id, type));
